...
class FM4MSGTool:
    def __init__(self, root):
        ...
        self.quickbms_script_path = str(Path("front_mission_ps2_pos_dat.bms"))
        self.quickbms_extract_input_path_var = tk.StringVar()
        self.quickbms_extract_output_path_var = tk.StringVar(value=str(Path.cwd() / "extracted-dat"))
        self.quickbms_build_input_path_var = tk.StringVar()
        self.quickbms_build_output_path_var = tk.StringVar()
        self.setup_gui()
        self.load_config()
    ...
    def setup_gui(self):
        ...
        for i, (text, command) in enumerate([
            ("-QuickBMS-", lambda: self.show_tab(0)),
            ("-MSG-", lambda: self.show_tab(1)),
            ("-ALPH-", lambda: self.show_tab(2)),
            ("-01FF-", lambda: self.show_tab(3)),
            ("-02FF-", lambda: self.show_tab(4)),
            ("-Настройки-", lambda: self.show_tab(5))
        ]):
    ...
    def setup_quickbms_tab(self):
        frame = tk.Frame(self.notebook)
        self.tabs.append(frame)

        script_frame = tk.LabelFrame(frame, text="Скрипт QuickBMS")
        script_frame.pack(fill=tk.X, padx=5, pady=5)
        self.quickbms_script_var = tk.StringVar(value=self.quickbms_script_path)
        script_entry = tk.Entry(script_frame, textvariable=self.quickbms_script_var, width=50)
        script_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5, pady=5)
        ...
        extract_input.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)
        self.text_context_menu.bind_to_widget(extract_input)
        tk.Button(extract_frame, text="Файл", command=self.select_quickbms_extract_input, width=8).grid(row=0, column=2, padx=5)
        tk.Label(extract_frame, text="Выходная папка:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        extract_output = tk.Entry(extract_frame, textvariable=self.quickbms_extract_output_path_var, width=40)
        extract_output.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=5)
        self.text_context_menu.bind_to_widget(extract_output)
        tk.Button(extract_frame, text="Обзор", command=self.select_quickbms_extract_output, width=10).grid(row=1, column=2, padx=5)
        tk.Button(extract_frame, text="ИЗВЛЕЧЬ из DVDIMAGE.DAT",
                  command=self.start_quickbms_extract,
                  bg=self.colors["accent_g"], font=('Arial', 10, 'bold'), height=2).grid(
            row=2, column=0, columnspan=3, pady=10, sticky=tk.EW)
        tk.Label(extract_frame, text="Файл DVDIMAGE.POS должен находиться в той же папке, что и DVDIMAGE.DAT",
                 font=('Arial', 8)).grid(row=3, column=0, columnspan=3)

        build_frame = tk.LabelFrame(frame, text="Замена файлов в DVDIMAGE.DAT/POS")
        build_frame.pack(fill=tk.X, padx=5, pady=5)
        tk.Label(build_frame, text="Папка с файлами:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        build_input = tk.Entry(build_frame, textvariable=self.quickbms_build_input_path_var, width=40)
        build_input.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)
        self.text_context_menu.bind_to_widget(build_input)
        tk.Button(build_frame, text="Обзор", command=self.select_quickbms_build_input, width=10).grid(row=0, column=2, padx=5)
        tk.Label(build_frame, text="Файл DVDIMAGE.DAT:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        build_output = tk.Entry(build_frame, textvariable=self.quickbms_build_output_path_var, width=40)
        build_output.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=5)
        self.text_context_menu.bind_to_widget(build_output)
        tk.Button(build_frame, text="Файл", command=self.select_quickbms_build_output, width=8).grid(row=1, column=2, padx=5)
        tk.Button(build_frame, text="ЗАМЕНА в DAT/POS",
                  command=self.start_quickbms_build,
    ...
    def run_quickbms(self, args):
        quickbms_exe = Path("quickbms_win/quickbms.exe")
        if not quickbms_exe.exists():
            messagebox.showerror("Ошибка", f"Файл {quickbms_exe} не найден")
            return False
        if args:
            script_path = Path(args[0])
            try:
                rel_script = script_path.relative_to(quickbms_exe.parent)
                args = [str(rel_script)] + list(args[1:])
            except ValueError:
                pass
        cmd = [str(quickbms_exe)] + args
        self.log_message(f"Запуск: {' '.join(map(str, cmd))}")
        try:
            result = subprocess.run(cmd, cwd=quickbms_exe.parent)
            if result.returncode in [0, 3221225786]:
                self.log_message("[✓] QuickBMS завершён успешно")
                return True
            else:
                self.log_message(f"[X] QuickBMS завершился с кодом {result.returncode}")
                messagebox.showerror("QuickBMS", f"Процесс завершился с ошибкой (код {result.returncode})")
                return False
        except Exception as e:
            self.log_message(f"[X] Не удалось запустить QuickBMS: {e}")
            messagebox.showerror("Ошибка запуска", str(e))
            return False

    def start_quickbms_extract(self):
        dat = Path(self.quickbms_extract_input_path_var.get())
        out = Path(self.quickbms_extract_output_path_var.get())
        script = Path(self.quickbms_script_var.get())
        if not dat.is_file():
            messagebox.showerror("Ошибка", "Укажите существующий DVDIMAGE.DAT")
            return
        # if not script.is_file():
            # messagebox.showerror("Ошибка", "Укажите существующий скрипт .bms")
            # return
        out.mkdir(parents=True, exist_ok=True)
        if not self.run_quickbms([str(script), str(dat), str(out)]):
            return
        messagebox.showinfo("Готово", "Извлечение завершено")

    def start_quickbms_build(self):
        folder = Path(self.quickbms_build_input_path_var.get())
        dat = Path(self.quickbms_build_output_path_var.get())
        script = Path(self.quickbms_script_var.get())
        if not folder.is_dir():
            messagebox.showerror("Ошибка", "Укажите папку с файлами")
            return
        if not dat.is_file():
            messagebox.showerror("Ошибка", "Укажите существующий DVDIMAGE.DAT")
            return
        # if not script.is_file():
            ...
            # return
        reimport_flag = ["-w"]
        if self.quickbms_reimport_mode.get() == 1:
            reimport_flag += ["-r"]
        elif self.quickbms_reimport_mode.get() == 2:
            reimport_flag += ["-r", "-r"]
        elif self.quickbms_reimport_mode.get() == 3:
            reimport_flag += ["-r", "-r", "-r"]
        if not self.run_quickbms(reimport_flag + [str(script), str(dat), str(folder)]):
            return
        messagebox.showinfo("Готово", "Замена завершена")
    
    def select_quickbms_script(self):
        f = filedialog.askopenfilename(title="Выберите скрипт QuickBMS", filetypes=[("BMS files", "*.bms")])
        if f: self.quickbms_script_var.set(f)

    def select_quickbms_extract_input(self):
        f = filedialog.askopenfilename(title="Выберите DVDIMAGE.DAT", filetypes=[("DAT files", "*.dat")])
        if f:
            self.quickbms_extract_input_path_var.set(f)
            if not self.quickbms_extract_output_path_var.get():
                self.quickbms_extract_output_path_var.set(str(Path(f).parent / "extracted-dat"))

    def select_quickbms_extract_output(self):
        d = filedialog.askdirectory(title="Выберите папку для извлечённых файлов")
        if d: self.quickbms_extract_output_path_var.set(d)

    def select_quickbms_build_input(self):
        d = filedialog.askdirectory(title="Выберите папку с файлами для сборки")
        if d: self.quickbms_build_input_path_var.set(d)

    def select_quickbms_build_output(self):
        f = filedialog.askopenfilename(title="Выберите DVDIMAGE.DAT для перезаписи", filetypes=[("DAT files", "*.dat")])
        if f: self.quickbms_build_output_path_var.set(f)
    ...
    def save_config(self):
        try:
            ...
            config = {
                ...
                'use_jis0208': self.use_jis0208_var.get(),
                'quickbms_script': self.quickbms_script_var.get(),
                'quickbms_extract_input': self.quickbms_extract_input_path_var.get(),
                'quickbms_extract_output': self.quickbms_extract_output_path_var.get(),
                'quickbms_build_input': self.quickbms_build_input_path_var.get(),
                'quickbms_build_output': self.quickbms_build_output_path_var.get(),
                'quickbms_reimport_mode': self.quickbms_reimport_mode.get(),
    ...
    def load_config(self):
        try:
            if self.config_path.exists():
                ...
                self.quickbms_script_var.set(config.get('quickbms_script', self.quickbms_script_path))
                self.quickbms_extract_input_path_var.set(config.get('quickbms_extract_input', ''))
                self.quickbms_extract_output_path_var.set(config.get('quickbms_extract_output', str(Path.cwd() / "extracted-dat")))
                self.quickbms_build_input_path_var.set(config.get('quickbms_build_input', ''))
                self.quickbms_build_output_path_var.set(config.get('quickbms_build_output', ''))
                self.quickbms_reimport_mode.set(config.get('quickbms_reimport_mode', 2))
